...
LabelHarmonization <- function(seurat.objects = c(), perform_integration = TRUE, integrated.atlas = NULL, downsample = TRUE, downsample_to = 100, npcs = 30, resolution = 0.5, classification.method = c("randomForest", "SVM", "Ensemble"), crossvalidationSVM = 10, validatePredictions = TRUE, selectanchorfeatures = 2000, ntree = 500) {
    ...
    if(classification.method == "randomForest")
    {
        ...
        rf_Celltypes.1 = randomForest_predictor(train = train, test = test, train_label = train_label, test_label = test_label, ntree = ntree)
...
DeduceRelationship <- function(reference1 = NULL, reference2 = NULL, downsample = FALSE, downsample_to = 100, classification.method = c("randomForest", "SVM", "Ensemble"), crossvalidationSVM = 5, selectvarfeatures = 2000, ntree = 500) {
  if(downsample == TRUE)
  {
  ...
  if(classification.method == "randomForest")
  {
    message ("Setting up randomForest classifier learning.")
    message ("Training randomForest classifier")
    rf_Celltypes.1 = randomForest_predictor(train = X$reference1[,1:num_features], test = X$reference2[,1:num_features], train_label = X$reference1$Celltypes, test_label = X$reference2$Celltypes, ntree = ntree)
...
ApproximationBasedCelltypeAssignment <- function(reference = reference, query = query, downsample = downsample, downsample_to = downsample_to, classification.method = classification.method, crossvalidationSVM = crossvalidationSVM, validatePredictions = validatePredictions, selectvarfeatures = selectvarfeatures, ntree = ntree) {
    ...
    if(classification.method == "randomForest")
    {
        message ("Setting up randomForest classifier learning")
        message ("Training randomForest classifier")
        rf_Celltypes.1 = randomForest_predictor(train = X$reference[,1:num_features], test = X$query[,1:num_features], train_label = X$reference$Celltypes, test_label = X$query$Clusters, ntree = ntree)
        message ("Predicting using trained randomForest classifier")
...
ClassifyCells <- function(reference = reference, query = query, downsample = downsample, downsample_to = downsample_to, classification.method = classification.method, crossvalidationSVM = crossvalidationSVM, validatePredictions = validatePredictions, selectvarfeatures = selectvarfeatures, ntree = ntree) {
    query_use = query
    ...
    if(classification.method == "randomForest")
    {
        ...
        query$PredictedCelltype_UsingRF <- rf_pred.1
        message ("Added Predicted celltypes using randomForest to query")

        if(validatePredictions == TRUE)
        {
            message("randomForest based learning and celltype annotation completed. Starting validation of celltype assignments using GSEA")
            reference.validation.use <- subset(reference_use, idents = query$PredictedCelltype_UsingRF)
            validation = ValidatePredictions(reference = reference.validation.use, query = query_use)
            message ("Validation completed. Please see summary of GSEA below")
            print (validation)
            write.table(validation, "Summary_GeneSetEnrichmentAnalysis.txt", quote=F, sep="\t")
    ...
    if(classification.method == "SVM")
    {
        ...
        query$PredictedCelltype_UsingSVM <- svm_pred.1
        message ("Added Predicted celltypes using SVM to query")

        if(validatePredictions == TRUE)
        {
            message("SVM based learning and celltype annotation completed. Starting validation of celltype assignments using GSEA")
            reference.validation.use <- subset(reference_use, idents = query$PredictedCelltype_UsingSVM)
            validation = ValidatePredictions(reference = reference.validation.use, query = query_use)
            message ("Validation completed. Please see summary of GSEA below")
            print (validation)
            write.table(validation, "Summary_GeneSetEnrichmentAnalysis.txt", quote=F, sep="\t")
    ...
    if(classification.method == "Ensemble")
    {
        ...
        predictions = data.frame(rf_pred.1, svm_pred.1)
        predictions$rf_accuracy = rep(rf_acccuracy_estimate.1, nrow(predictions))
        predictions$svm_accuracy = rep(svm_accuracy_estimate.1, nrow(predictions))
        predictions$Match = predictions$rf_pred.1 == predictions$svm_pred.1
        predictions$final_pred <- ifelse(as.character(predictions$Match) == 'TRUE', as.character(predictions$rf_pred.1), ifelse(predictions$rf_accuracy > predictions$svm_accuracy, as.character(predictions$rf_pred.1), as.character(predictions$svm_pred.1)))
        ...
        {
            message("Ensembl celltype annotation completed. Starting validation of celltype assignments using GSEA")
            reference.validation.use <- subset(reference_use, idents = query$PredictedCelltype_UsingEnsemble)
            validation = ValidatePredictions(reference = reference.validation.use, query = query_use)
            message ("Validation completed. Please see summary of GSEA below")
            print (validation)
            write.table(validation, "Summary_GeneSetEnrichmentAnalysis.txt", quote=F, sep="\t")
